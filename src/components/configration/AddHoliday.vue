    ...
    <div class="w-[90%] h-[85%] mt-10 bg-ss-body rounded-lg flex flex-col items-start justify-start px-8 py-6">
      ...
      <div v-else class="w-full h-full mt-8 flex flex-col items-center justify-start text-white">
        <div class="w-full flex items-center justify-start flex-wrap">
          <div v-for="holiday in holidayStore.allHolidays" :key="holiday._id"
            class="holiday-block relative group w-[80px] h-[80px] mx-2 flex flex-col items-center justify-center rounded-md border border-ss-light-purple p-2 cursor-pointer">
            ...
            <span class="bg-white text-sm text-center w-[180px] p-2 text-black absolute top-[-50px]  hidden group-hover:flex items-center justify-center rounded-sm">{{ holiday.name }}</span>
          </div>
          <div
            class="w-[80px] h-[80px] mx-2 flex flex-col items-center justify-center rounded-md border border-dashed border-ss-light-purple cursor-pointer "
            @click="holidayStore.toogleHolidayModal">
            <img
              src="https://res.cloudinary.com/qtalk/image/upload/v1677843878/SuperLeaves/icons8-plus-math-50_q28sji.png"
              class="w-[30px] h-[30px]" alt="add">
          </div>
        </div>
